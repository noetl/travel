...
NoETL returns `output.data.rows[].result`, a JSON object per row. The original `int2` decoder issue is fixed in the merged tool revision used by direct validation; JSON row projection remains compatible with the pinned worker. Monetary prices stay integer cents; exchange rates use exact numeric storage. API consumers must preserve 64-bit IDs (JavaScript numbers cannot represent every bigint); production client envelopes should encode large IDs as strings before exposure.
