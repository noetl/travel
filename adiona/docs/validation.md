...
## Hospitality extension validation

The direct suite uses the merged tools revision `8048b14169f3bbaea6430b0b9762ae2cdfe4d8c6`. `hospitality.mjs` exercises actual SQL through that tool; `--runtime` dispatches the same workflows through the pinned Rust server and worker. Coverage includes version-2 replay/checksum guards, capacity/date errors, exact quotes, price snapshots, overlapping and adjacent stays, permanent idempotency keys, inactive inventory, provider/guest isolation, immutable snapshot privileges, expiry, lifecycle/version checks, atomic FK failures and competing holds. On PostgreSQL 19beta3, **24 direct hospitality checks** and **23 full-runtime hospitality checks (61 executions, including expected failures)** passed. The existing 37 catalog checks and 21 catalog runtime executions also passed. Detailed counts and execution IDs are in `hospitality-tool-results.json` and `hospitality-runtime-results.json`.

The new `Adiona PostgreSQL 19` CI workflow builds the checksummed PostgreSQL source and pinned Rust adapter, then runs the catalog and hospitality database suites. Full server/worker coverage remains a separately recorded local validation, not a claim about that CI job.
