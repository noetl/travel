...
if [[ ! -f "$root/.local/pgdata/PG_VERSION" ]]; then
  "$pg_bin/initdb" -D "$root/.local/pgdata" -U migration_owner -A trust --no-locale -E UTF8 > "$root/.local/initdb.log"
fi
"$pg_bin/pg_ctl" -D "$root/.local/pgdata" -l "$root/.local/postgres.log" -o "-h 127.0.0.1 -p 55439 -k ''" start
